...
{{-- <div class="row">
    ...
    <x-inputs.group class="col-sm-6">
        <x-inputs.number
            name="envoye"
            label="Envoye"
            :value="old('envoye', ($editing ? $transaction->envoye : '400'))"
            step="0.01"
            placeholder="Envoye"
            required
        ></x-inputs.number>
    ...
    <x-inputs.group class="col-sm-4">
        <x-inputs.number
            name="frais"
            label="Frais"
            :value="old('frais', ($editing ? $transaction->frais : '45'))"
            step="0.01"
            placeholder="Frais"
            required
        ></x-inputs.number>
    ...
    <x-inputs.group class="col-sm-4">
        <x-inputs.number
            name="taux"
            label="Taux"
            :value="old('taux', ($editing ? $transaction->taux : (Auth()->user()->location == 'mali' ? 62.5 : 60.5)))"
            step="0.01"
            placeholder="Taux"
            required
        ></x-inputs.number>
